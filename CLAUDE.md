...
- **`shared/lifecycle.py`** — the notify → track message-ids → recheck-batch → status-transition → delete + batched replaceable-summary lifecycle, parameterised by DB accessors, status vocabulary (verkocht/onder bod vs verhuurd/onder optie), fetch hook, and summary text. `reads_gone`/`is_gone` implement the page-scoped, conservative gone/sold detection (strip sidebar/footer carousels; trust unambiguous page-status phrases anywhere; trust bare status badges only inside the `<h1>` header region; 404/410 counts as gone). `run_recheck` is the round-robin loop (cursor advanced *before* the fetch). `send_replaceable_summary` deletes the prior summary and sends the new one.
